...
class DatabaseGame
{
    ...
    {
        DB::table('worlds')->where('id', $worldID)->update(['is_occupied' => 1]);
    }

    public function addVillage($worldID, $userID, $username, $capital)
    {
        $total = $this->getVillagesID($userID);
    ...
    {
        return DB::table('villages')->where('user_id', $userID)->orderByDesc('is_capital')->count();
    }

    public function addResourceFields($worldID, $type)
    {
        $fieldValues = [
            1 => ['f1t' => 3], 2 => ['f2t' => 3], 3 => ['f3t' => 3], 4 => ['f4t' => 3], 5 => ['f5t' => 3], 6 => ['f6t' => 4],
            7 => ['f7t' => 1], 8 => ['f8t' => 3], 9 => ['f9t' => 3], 10 => ['f10t' => 3], 11 => ['f11t' => 3], 12 => ['f12t' => 3]
        ];
        ...
        DB::table('fields')->insert(array_merge(['world_id' => $worldID], $fields));
    }

    public function addUnits($worldID)
    {
        DB::table('units')->insert(['world_id' => $worldID]);
    }
    ...
    public function getVillageType($worldID)
    {
        return DB::table('worlds')->select('field_type')->where('id', $worldID)->first()->field_type;
    }

    public function getWref($x, $y)
    {
        return DB::table('worlds')->select('id')->where(['x' => $x, 'y' => $y])->first()->id;
    }
    ...
    {
        $result = DB::table('worlds')->select('oasis_type', 'is_occupied')->where('id', $worldID)->first();
    ...
    public function getVillageStateForSettle($worldID)
    {
        ...
        if ($result['is_occupied'] == 0 && $result['oasistype'] == 0 && $result['fieldtype'] == 0) {
            return true;
        }
        return false;
    }
    ...
    public function clearTech($worldID)
    {
        $this->conn->delete('tdata', 'vref = :vref', ['vref' => $worldID]);
        $this->addTech($worldID);
    }

    public function addTech($worldID)
    {
        DB::table('technologies')->insert(['world_id' => $worldID]);
    }
    ...
    public function addABTech($worldID)
    {
    ...
    public function getOMInfo($worldID)
    {
        return DB::table('worlds')
            ->leftJoin('oases', 'oases.world_id', '=', 'worlds.id')
            ->where('worlds.id', $worldID)
            ->first();
    }

    public function getMInfo($worldID)
    {
        return DB::table('worlds')
            ->leftJoin('villages', 'villages.world_id', '=', 'worlds.id')
            ->where('worlds.id', $worldID)
            ->first();
    }
